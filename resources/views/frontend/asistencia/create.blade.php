    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.asistencia.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="n_identificacions[]" id="n_identificacions" multiple>
                                @foreach($n_identificacions as $id => $n_identificacion)
                                    <option value="{{ $id }}" {{ in_array($id, old('n_identificacions', [])) ? 'selected' : '' }}>{{ $n_identificacion }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="asistio" id="asistio" value="1" {{ old('asistio', 0) == 1 ? 'checked' : '' }}>
